    ...
    <Layout style={layoutStyle}>
      <Header style={headerStyle}>
        <div className='header'>
          ...
          <Flex gap='middle' align='center' className='header__buttons'>
            {["/", "/register", "/register/"].includes(location.pathname) && (
              <NavLink to='/login'>
                <Button className='button' size={size}>Вход</Button>
              </NavLink>
            )}
            {["/", "/login", "/login/"].includes(location.pathname) && (
              <NavLink to='/register'>
                <Button className='button' size={size}>Регистрация</Button>
              </NavLink>
            )}
            {["/admin", "/storage", "/admin/", "/storage/"].includes(location.pathname) && (
              <>
                {currentUser && <span className='welcome-text'>Добро пожаловать, {currentUser.username}</span>}
                <NavLink to='/'>
                  <Button className='button' size={size} onClick={handleLogout}>Выход</Button>
                </NavLink>
              </>
            )}
          </Flex>
